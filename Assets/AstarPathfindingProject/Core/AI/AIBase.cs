...
namespace Pathfinding {
	...
	public abstract class AIBase : VersionedMonoBehaviour {
		 ...
		 *
		 * \see #gravity
		 * \deprecated Use the #height property instead (2x this value)
		 */
		...
		protected Vector3 RaycastPosition (Vector3 position, float lastElevation) {
			...
			float rayLength = height * 0.5f + Mathf.Max(0, lastElevation-elevation);
		...
		protected virtual void OnDrawGizmos () {
			...
			if (orientation == OrientationMode.YAxisForward) {
				Draw.Gizmos.Cylinder(position, Vector3.forward, 0, radius, color);
			} else {
				Draw.Gizmos.Cylinder(position, rotation * Vector3.up, height, radius, color);
			}
