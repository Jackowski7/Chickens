...
namespace Pathfinding {
	...
	public interface IAstarAI {
		 ...
		 *
		 * \note The #Pathfinding.AILerp script doesn't really have any use of knowing the radius or the height of the character, so this property will always return 0 in that script.
		 */
		 ...
		 *
		 * \note This is the distance to the end of the path, which may or may not be at the #destination. If the character cannot reach the destination it will try to move as close as possible to it.
		 *
		 ...
		 *
		 * \see #reachedDestination
		 * \see #reachedEndOfPath
		 */
		float remainingDistance { get; }

		/** True if the ai has reached the #destination.
		 * This is a best effort calculation to see if the #destination has been reached.
		 * For the AIPath/RichAI scripts, this is when the character is within \link AIPath.endReachedDistance endReachedDistance\endlink world units from the #destination.
		 ...
		 *
		 * Furthermore it will not report that the destination is reached if the destination is above the head of the character or more than half the #height of the character below its feet
		 * (so if you have a multilevel building, it is important that you configure the #height of the character correctly).
		 *
