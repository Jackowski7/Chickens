...
namespace Pathfinding {
	...
	public class AstarData {
		...
		/** Calls the \a callback with every node in all graphs.
		 * This is the easiest way to iterate through every existing node.
		 *
		 * \snippet MiscSnippets.cs AstarData.GetNodes1
		 *
		 * \see #Pathfinding.NavGraph.GetNodes for getting the nodes of a single graph instead of all.
