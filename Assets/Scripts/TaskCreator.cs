...
public class TaskCreator : MonoBehaviour
{
	...
	List<GameObject> EvaluateTargets(Chicken chicken, Action action, Resource resource)
	{
		...
		{
			Info TargetInfo = targets[x].GetComponent<Info>();


			if (action == Action.Sleep) // if we're sleeping
			{
				if (TargetInfo.actions.Contains(Action.Sleep)) // if this place lets us sleep
				{
					if (TargetInfo.open && TargetInfo.chickens.Count < TargetInfo.maxSpots)// if it's open and has space
			...
			{
				if (TargetInfo.actions.Contains(Action.Get)) // if this place let's us get
				{
					if (TargetInfo.inventory[(int)resource].x > 0) // and it has some of this resource
					{
						if (TargetInfo.open && TargetInfo.chickens.Count < TargetInfo.maxSpots)// if it's open and has space
							potentialTargets.Add(targets[x]);  // add it to the list to consider
					}
			...
			if (action == Action.Put) // if we're putting something
			{
				...
				if (TargetInfo.inventory[(int)resource].x < TargetInfo.inventory[(int)resource].y)
				{
					if (TargetInfo.open && TargetInfo.chickens.Count < TargetInfo.maxSpots)// if it's open and has space
						potentialTargets.Add(targets[x]);  // add it to the list to consider
				}
			...
			{
				if (TargetInfo.actions.Contains(Action.Make) && TargetInfo.resourceProduced == resource) // if this place lets us make what we're making
				{
					...
					else
					{
						...
						{
							DropOffResources(chicken, TargetInfo.gameObject); // put the materials here
						}
					}
				}
			}
			...
			{
				if (TargetInfo.actions.Contains(Action.Forester) && TargetInfo.resourceProduced == resource) // if this place lets us make what we're making
				{
					if (TargetInfo.open && TargetInfo.chickens.Count < TargetInfo.maxSpots)// if it's open and has space
						potentialTargets.Add(targets[x]);  // add it to the list to consider
				}
			...
			{
				if (TargetInfo.actions.Contains(Action.PlantTree))
				{
					if (TargetInfo.open && TargetInfo.chickens.Count < TargetInfo.maxSpots)// if it's open and has space
			...
			{
				if (TargetInfo.actions.Contains(Action.ChopTree))
				{
					if (TargetInfo.open && TargetInfo.chickens.Count < TargetInfo.maxSpots)// if it's open and has space
						potentialTargets.Add(targets[x]);  // add it to the list to 
				}
			...
			{
				if (TargetInfo.actions.Contains(Action.Build)) // if this place lets us build
				{
					...
					for (int y = 0; y < TargetInfo.constructionMaterials.Count; y++)
					{
						...
						{
							hasMaterials = false;
							amountNeeded = TargetInfo.constructionMaterials[y] - (int)TargetInfo.inventory[y].x;
							resourceNeeded = (Resource)y;
						}
					}
			...
			{
				if (TargetInfo.actions.Contains(Action.Make)) // if this place lets us make what we're making
				{
					bool hasIngredients = true;
					for (int y = 0; y < TargetInfo.actionIngredients.Count; y++)
					{
						if (TargetInfo.inventory[y].x < TargetInfo.actionIngredients[y])
						{
							hasIngredients = false;
						}
					}
				...
				if (TargetInfo.actions.Contains(Action.Build))
				{
					bool hasMaterials = true;
					for (int y = 0; y < TargetInfo.constructionMaterials.Count; y++)
					{
						if (TargetInfo.inventory[y].x < TargetInfo.constructionMaterials[y])
						{
							hasMaterials = false;
						}
					}
